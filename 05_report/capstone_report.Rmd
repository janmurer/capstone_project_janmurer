...
The following block of code creates a wrapper function to access and interact with the OpenCage API. The function takes a user location (address) as input and returns the coordinates of the location. The user coordinates are stored in a data frame for further processing. The wrapper function does not contain a "Sys.sleep()" expression, as the application should not call the API excessively.
...
The following function is used to find the nearest location to the current user's location. The geographical distance between two points is calculated using the Euclidean distance formula. The function takes the user coordinates and a data frame with locations as input. After calculating the distance between the user coordinates and each location in the data frame, the function returns the name of the nearest location. 
...
The coordinates stored in the .json file are in the EPSG:2056 coordinate system, whereas the coordinates received from the OpenCage API are in the World Geodetic System (WGS 84). To compare the coordinates and calculate the distance, coordinates in EPSG:2056 need to be converted to WGS 84. This is done using the sf package. 
...
The application was entirely coded and troubleshooted using ChatGPT and Microsoft Copilot. Therefore, the code may show inconsistencies upon closer inspection. Although the code was tested, it may not adhere to coding best practices and conventions.
