...
As a capstone project for the seminar "Data Mining for the Social Sciences in R" i created a small shiny application that takes the current user location/address and shows the nearest available public toilet. 

The application uses the OpenCage API to get coordinates from an address (geocoding) and the opendata.swiss API to get a dataset of all public toilettes in Lucerne.

The application is structured in three different scripts:

  - functions.R: This script contains wrapper functions to interact with API's and a function to calculate the geographical distance between two points.
...
Further i will disect each script and explain the code in detail.  
...
Set up API keys and base URL for OpenCage API. Loading the API key from a local .csv file keeps the key secret. This way, the key does not show up in the code and can be excluded in the .gitignore file (therefore it is not being pushed when working with git).
...
The following block of code creates a wrapper function to access and interact with the OpenCage API. The function takes an user location (address) as input and returns the coordinates of the location. The user coordinates are stored in a data frame for further processing. The wrapper function does not contain a Sys.sleep() function, as the application should not call the API excessively.
...
The application would also work with a locally stored dataset and therefore be more robust and efficient. As the list of locations potentionally could be updated in the future, the application fetches the most recent dataframe. 
...
fetch_opendata_swiss <- function(dataset_id) {
  ...
  if (http_type(response) == "application/json") {
    ...
    if (!is.null(json_content$result$resources)) {
      ...
      # Extract the resource URL for the dataset you want to download and store in a variable
      ...
      # Unzip the dataset to "01_data_input" folder using base R function and here
...
The following function is used to find the nearest location to the current user location. The geographical distance between two points is calculated using the Euclidean distance formula. The function takes the user coordinates and a dataframe with locations as input. After calculating the distance between the user coordinates and each location in the dataframe, the function returns the name of the nearest location. 
...
In a next step, the data is loaded from the .json file. As the data comes in a nested structure, with the features of each location separated from its respective coordinates, the script extracts properties and coordinates from the .json. Because both x and y coordinates are stored as one value in the fetched source file, the script continues to separate the coordinates into two values. Finally, the script combines the properties of each location, the x coordinate and the y coordinate into a single dataframe. 
...
After storing the data in a dataframe, the script continues to perform basic data cleaning and filtering. 
...
The coordinates stored in the .json file are in the EPSG:2056 coordinate system, where as the coordinates received from the OpenCage API are in the World Geodetic System (WGS 84). In order to compare the coordinates and calculate the distance, coordinates in EPSG:2056 need to be converted to WGS 84. This is done using the sf package. 
...
The script "waits" for the user enter a location and press the "Find Nearest Toilet" button. When the button is clicked, the functions get_user_coordinates() and find_nearest_location() are called to find the nearest public toilet. 
...
The application wont work as a standalone application, as the API key for the OpenCage API is not included in the repository. 

#### Fragile Code

The design of the code is rather fragile. The code may break if the API's change or the structure of the dataset changes. 
...
#### The use of LLM's for coding

The application was entirely coded using ChatGPT and Microsoft Copilot. Therefore the code may show inconsistencies up on closer inspection. Although the code was tested, it may not stick to coding best practices and conventions. 
...
The application could be changed to run on the web using services like shinyapps.io. This would make the application more useable and accessible for phones. 
...
"SeatSeekr" is a rather primitive application that finds the nearest public toilet in Lucerne based on the users given location. 

Given the fact that i had no prior experience with coding in general, i am quite happy with the results.
...
- Simmilar project: https://map.stadtluzern.ch/wc-app/